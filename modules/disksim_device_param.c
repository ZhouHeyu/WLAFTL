...
TOP:
	pnum = lp_param_name(lp_mod_name("disksim_device"), b->params[c]->name);
	...
	if(stack_ptr > 0) {
	    for(c = 0; c < b->params_len; c++) {
	        if(lp_param_name(lp_mod_name("disksim_device"), b->params[c]->name) == needed)
		    goto FOUND;
            }
	    break;
	}
